      ...
      setMessages([
        {
          role: "assistant",
          content:
            "Hi! I'm your AI trip planner. Tell me where you'd like to travel and I'll help you create the perfect itinerary. For example: 'I want to visit West Bengal for 5 days in March'",
          timestamp: new Date(),
        },
      ]);
    }
  ...
  const loadExistingSession = async (sessionId: string) => {
    try {
      ...
      const response = await axios.get(
        `/api/planner/session?sessionId=${sessionId}`,
      );
      ...
      if (sessionData.approvedPlaces && sessionData.approvedPlaces.length > 0) {
        console.log(
          "FRONTEND: Loading approved places:",
          sessionData.approvedPlaces,
        );

        // Fetch place details for approved places
        try {
          const placesResponse = await axios.post(
            "/api/planner/approved-places",
            {
          ...
          console.log(
            "FRONTEND: Approved places details:",
      ...
      if (sessionData.finalizedItinerary) {
        console.log(
          "FRONTEND: Loading finalized itinerary:",
          sessionData.finalizedItinerary,
        );
      ...
      if (
        ...
        const messagesWithDates = sessionData.conversationHistory.map(
          (msg: any) => ({
            ...msg,
        ...
        setMessages([
          {
            role: "assistant",
            content:
              "Welcome back! Continue planning your trip or start a new conversation.",
            timestamp: new Date(),
          },
        ]);
      }
      ...
      setMessages([
        {
          role: "assistant",
          content:
            "Hi! I'm your AI trip planner. Tell me where you'd like to travel and I'll help you create the perfect itinerary.",
          timestamp: new Date(),
        },
      ]);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessages((prev) => {
        ...
        console.log(
          "Last message places:",
          newMessages[newMessages.length - 1].places,
        );
        return newMessages;
      });
      ...
      if (action === "approve") {
        ...
        setMessages((prev) => {
          ...
          if (updated[messageIndex]?.places) {
            updated[messageIndex] = {
              ...updated[messageIndex],
              places: updated[messageIndex].places!.filter(
                (p) => p.placeId !== placeId,
              ),
            };
          }
            ...
            {finalItinerary && finalItinerary.daySchedules && (
              <div style={{ marginBottom: "24px", marginTop: "32px" }}>
                ...
                >
                  ...
                  >
                    {finalItinerary.totalPlaces} places • {finalItinerary.days}{" "}
                    days • {finalItinerary.destination}
                  </div>
                </div>
                ...
                >
                  {finalItinerary.daySchedules.map(
                    (daySchedule: any, dayIndex: number) => (
                      <div
                        key={daySchedule.day}
                        style={{
                          marginBottom:
                            dayIndex < finalItinerary.daySchedules.length - 1
                              ? "48px"
                              : "0",
                        }}
                      >
                        ...
                        >
                          📅 Day {daySchedule.day}{" "}
                          {daySchedule.date ? `- ${daySchedule.date}` : ""}
                          <div
                            style={{
                              fontSize: "14px",
                              fontWeight: "normal",
                          ...
                          >
                            {daySchedule.places.length} places •{" "}
                            {daySchedule.totalDuration} min •{" "}
                            {daySchedule.totalDistance.toFixed(2)} km
                          </div>
                        </div>
                        ...
                        {daySchedule.places.map((item: any, index: number) => (
                          <div
                            key={`${daySchedule.day}-${item.place.placeId}-${index}`}
                            style={{
                              marginBottom:
                ...
                >
                  ...
                  >
                    🎉 Have an amazing trip! Save this itinerary or share it
        ...
        >
          ...
          >
            ...
            >
              ...
              {approvedPlaces.length === 1 ? "place" : "places"} selected
            </div>
