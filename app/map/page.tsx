...
function MapPageContent() {
  ...
  const [placeDetails, setPlaceDetails] = useState<MapPlaceDetails | null>(
    null,
  );
    ...
    async function getPlaces() {
      ...
      if (placeIdFromUrl) {
        const place = response.data.find(
          (place: Places) => place.placeId === placeIdFromUrl,
        );
        if (place) {
    ...
    <div className="relative w-full h-full overflow-hidden">
      {planningMode && !itinerary && (
        ...
        >
          ...
          <button
            ...
            style={{
              padding: "6px 12px",
              background: selectedPlaces.length < 2 ? "#ccc" : "#000",
              color: "#fff",
              borderRadius: "4px",
              border: "none",
              cursor: selectedPlaces.length < 2 ? "not-allowed" : "pointer",
            }}
          >
            Chart Route
          </button>
              ...
              {[
                ...
                <label
                  ...
                  style={{
                    ...
                    padding: "12px",
                    border: "2px solid",
                    borderColor: transportMode === mode.value ? "#000" : "#ddd",
                    borderRadius: "8px",
                    marginBottom: "8px",
                    cursor: "pointer",
                  }}
                >
                  ...
                  <span
                    style={{
                      fontWeight:
                        transportMode === mode.value ? "bold" : "normal",
                    }}
                  >
                    {mode.label}
                  </span>
            ...
            <div style={{ display: "flex", gap: "8px" }}>
              ...
              <button
                ...
                style={{
                  ...
                  borderRadius: "8px",
                  border: "none",
                  cursor: generatingRoute ? "wait" : "pointer",
                }}
              >
                    ...
                    <Pin
                      background={
                        isSelected
                          ? "#F97316"
                          : (PLACE_TYPE_COLORS[
                              place.type as keyof typeof PLACE_TYPE_COLORS
                            ] ?? "#95A5A6")
                      }
                      glyphColor="white"
      ...
      {(selectedPlace || itinerary) && (
        ...
        >
          ...
          {/* header */}
          <div
            className="flex item-center justify-between border-b px-5 py-4"
            style={{
              marginTop:
                typeof window !== "undefined" && window.innerWidth < 768
                  ? "10px"
            ...
            {itinerary ? (
              <div style={{ padding: "20px" }}>
                ...
                >
                  ...
                  >
                    Your Route - {itinerary.route.length} places
                  </div>
                  <div
                    style={{
                      fontSize: "14px",
                  ...
                  >
                    Total: {Math.floor(itinerary.totalTime / 60)}h{" "}
                    {itinerary.totalTime % 60}m |{" "}
                    {(itinerary.totalDistance / 1000).toFixed(1)} km
                  </div>
                    ...
                    {item.type === "visit" ? (
                      ...
                      >
                        ...
                        <div style={{ fontSize: "12px", color: "#666" }}>
                          Visit duration: {item.visitDuration} min
                        </div>
                      </div>
                    ) : (
                      <div
                        ...
                        {item.options?.map((opt: any, optIdx: number) => (
                          ...
                          >
                            ...
                            <div
                              style={{
                                fontSize: "13px",
                                color: opt.mode === item.mode ? "#fff" : "#888",
                              }}
                            >
                              {opt.duration} min •{" "}
                    ...
                    {placeDetails.photos.length > 1 && (
                      <div className="flex gap-2">
                        {placeDetails.photos.slice(1, 4).map((photo, i) => (
                          <div
                            key={i}
                            className="flex-1 h-16 rounded-md overflow-hidden bg-muted"
                          >
                            <img
                              src={photo}
