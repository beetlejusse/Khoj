...
function extractTripInfo(messages: any[], currentSession: any) {
  ...
  // Pattern 1: "in [CITY]" - captures city names
  const inPattern = content.match(/\bin\s+([a-z]+(?:\s+[a-z]+)?)/gi);
  if (inPattern) {
    inPattern.forEach((match: string) => {
      const place = match.replace(/\bin\s+/i, '').trim();
      if (place && place.length > 2 && !['the', 'for', 'and', 'with'].includes(place)) {
  ...
  // Pattern 2: "visit/go to X" - captures specific places
  const visitPattern = content.match(/(?:visit|go to|going to|want to visit|want to go to|traveling to|trip to)\s+([a-z]+(?:\s+[a-z]+)?)/gi);
  if (visitPattern) {
    visitPattern.forEach((match: string) => {
      const place = match.replace(/(?:visit|go to|going to|want to visit|want to go to|traveling to|trip to)\s+/i, '').trim();
      if (place && place.length > 2 && !locations.includes(place)) {
        locations.push(place);
  ...
  const atPattern = content.match(/\b(?:at|near)\s+([a-z]+(?:\s+[a-z]+)?)/gi);
  if (atPattern) {
    atPattern.forEach((match: string) => {
      const place = match.replace(/\b(?:at|near)\s+/i, '').trim();
      if (place && place.length > 2 && !locations.includes(place)) {
        locations.push(place);
  ...
  const isRefining = !locations.length && currentSession?.destination && (
  ...
  if (content.includes('food') || content.includes('cuisine') || content.includes('restaurant') || content.includes('eat') || content.includes('thali') || content.includes('daal') || content.includes('churma')) interests.push('food', 'restaurant');
  if (content.includes('culture') || content.includes('heritage') || content.includes('history') || content.includes('authentic')) interests.push('culture', 'museum');
  if (content.includes('nature') || content.includes('park') || content.includes('wildlife')) interests.push('nature', 'park');
  if (content.includes('adventure') || content.includes('trek') || content.includes('hiking')) interests.push('adventure', 'tourist_attraction');
  if (content.includes('temple') || content.includes('church') || content.includes('mosque') || content.includes('masjid') || content.includes('religious')) interests.push('place_of_worship');
  if (content.includes('shopping') || content.includes('market') || content.includes('bazaar') || content.includes('chowk')) interests.push('shopping_mall', 'store');
  if (content.includes('palace') || content.includes('fort') || content.includes('monument')) interests.push('tourist_attraction', 'museum');
  if (content.includes('lake') || content.includes('river') || content.includes('water')) interests.push('tourist_attraction', 'park');
  if (content.includes('everything') || content.includes('all') || content.includes('anything')) interests.push('tourist_attraction', 'restaurant', 'museum');
  
  if (interests.length === 0) interests.push('tourist_attraction', 'restaurant');
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log('=== BACKEND: Extracted trip info ===', tripInfo);

    let shouldDiscoverPlaces = false;
    let discoveredPlaces = null;

    if (tripInfo && tripInfo.destination) {
      shouldDiscoverPlaces = true;
      ...
      // If multiple locations mentioned, search near all of them
      if (tripInfo.allLocations && tripInfo.allLocations.length > 1) {
        console.log('Multiple locations detected, searching near all:', tripInfo.allLocations);
        
        // Search near each location and combine results
        const allDiscoveredPlaces = [];
        for (const location of tripInfo.allLocations.slice(0, 3)) { // Limit to 3 locations
